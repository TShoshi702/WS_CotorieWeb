...
export default function HomePage({ onNavigate, onProductClick }: HomePageProps) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollTimeout = setTimeout(() => {
        ...
        // 如果接近某个板块（距离小于 15% 视口高度），则吸附过去
        if (closestSection && minDistance > 0 && minDistance < viewportHeight * 0.15) {
          const targetY = closestSection.offsetTop;
          // 根据距离调整动画时长，越远越慢
  ...
  const featuredProducts = products.filter(p => p.isNew || true).slice(0, 4);
  ...
  return (
    <div className="home-scroll-container" ref={containerRef} style={{ backgroundColor: '#faf6f2' }}>
          ...
          {featuredProducts.map((product) => (
            ...
            >
              <div className="pg">
                ...
                <div className="code">{product.collection.toUpperCase()} {product.id.split('-')[1]}</div>
              </div>
              <div className="product-info">
                <h3>{language === 'zh' ? product.name : language === 'ja' ? product.nameJa : product.nameEn}</h3>
                <span>{product.isNew ? (language === 'zh' ? '2025 春夏新品' : language === 'ja' ? '2025 春夏新作' : 'Spring/Summer 2025 New') : (language === 'zh' ? '限定系列' : language === 'ja' ? '限定シリーズ' : 'Limited Collection')}</span>
              </div>
            </a>
          ))}
        </div>
      ...
      {/* Brand Model Showcase - Pradasphere Style Dark Section */}
      <section style={{
        padding: '96px 0',
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.backgroundColor = '#c49a8c';
      ...
      <style>{`
        ...
        .home-scroll-container > .products {
