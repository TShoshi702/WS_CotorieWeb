...
export default function ProductDetailPage({ productId, onBack }: ProductDetailPageProps) {
  ...
  // Shopify 实时数据
  const { products, loading } = useAllProducts(100);
  const product = products.find((p) => p.id === productId) || null;

  const collectionHandle = product?.collection || '';
  const { products: collectionProducts } = useProductsByCollection(collectionHandle, 50);
  const recommendedProducts = product
    ? collectionProducts.filter((p) => p.id !== product.id).slice(0, 6)
    : [];
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <p style={{ fontSize: '13px', letterSpacing: '2px', color: '#666' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '64px', backgroundColor: '#faf6f2' }}>
      <button
        onClick={onBack}
        className="mobile-only"
        style={{
          display: 'none', position: 'fixed', top: '72px', left: '16px',
          zIndex: 100, background: 'rgba(250,246,242,0.95)',
      ...
      {addedToCart && (
        <div style={{
          ...
          transform: 'translateX(-50%)', backgroundColor: '#1a1a1a',
          color: '#fff', padding: '12px 32px', fontSize: '12px',
          letterSpacing: '2px', textTransform: 'uppercase',
          zIndex: 1500, animation: 'fadeInOut 2s ease-in-out',
        }}>
          {getLabel('added')}
        </div>
      )}
      ...
      {/* 50/50 Layout */}
      <div style={{
        display: 'grid', gridTemplateColumns: '1fr', gap: '0', width: '100%',
        ...
        <div
          ...
          style={{
            width: '50vw', position: 'sticky', top: '64px',
            height: 'calc(100vh - 64px)', overflowY: 'auto', backgroundColor: '#ffffff',
          }}
        >
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {product.images.map((img, idx) => (
              <div key={idx} style={{ width: '100%', aspectRatio: '0.75', backgroundColor: '#f8f8f8', overflow: 'hidden' }}>
                <img src={img} alt={`${getName()} ${idx + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Right - Product Info */}
        <div style={{ backgroundColor: '#ffffff', padding: '80px 4vw', minHeight: 'calc(100vh - 64px)' }}>
          <div style={{ maxWidth: '480px', margin: '0 auto' }}>
            <h1 style={{
              fontSize: '20px', fontWeight: 400, letterSpacing: '3px',
              textTransform: 'uppercase', marginBottom: '24px', color: '#000000', textAlign: 'left',
            }}>
              {getName()}
            </h1>

            <div style={{
              fontSize: '16px', fontWeight: 300, letterSpacing: '2px',
              color: '#000000', marginBottom: '32px', textAlign: 'left',
            }}>
              ¥{product.price.toLocaleString()}
            </div>

            <p style={{
              fontSize: '13px', lineHeight: 2, letterSpacing: '0.5px',
              color: '#666666', marginBottom: '40px', textAlign: 'left',
            }}>
              {getDescription()}
            </p>

            <div style={{ marginBottom: '24px' }}>
              <div style={{ fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px', color: '#000000' }}>
                {getLabel('color')}
              </div>
                ...
                {product.colors.map((color) => (
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s', minWidth: '60px',
                    }}
                  >
                    {color}
                  </button>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', transition: 'background-color 0.3s', marginBottom: '16px',
              }}
              onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#333')}
              onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = addedToCart ? '#333' : '#000')}
            >
              {addedToCart ? getLabel('added') : getLabel('addToCart')}
            </button>

            <p style={{ fontSize: '11px', color: '#666', textAlign: 'left', letterSpacing: '0.5px' }}>
            ...
            <div style={{ marginTop: '40px', borderTop: '1px solid #e0e0e0' }}>
              {['details', 'care', 'shipping'].map((section) => (
                <div key={section} style={{ borderBottom: '1px solid #e0e0e0' }}>
                  ...
                  >
                    <span style={{ fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: '#000000' }}>
                      {getLabel(section)}
                    </span>
                    {expandedSection === section ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                  </button>
                  {expandedSection === section && (
                    <div style={{ paddingBottom: '16px', fontSize: '12px', lineHeight: 1.8, color: '#666', letterSpacing: '0.5px' }}>
      ...
      {recommendedProducts.length > 0 && (
        <div style={{ backgroundColor: '#ffffff', borderTop: '1px solid #e0e0e0', padding: '64px 4vw' }}>
          <h3 style={{
            fontSize: '14px', fontWeight: 400, letterSpacing: '2px',
            textTransform: 'uppercase', marginBottom: '40px', color: '#000000',
          }}>
            {getLabel('youMayLike')}
          </h3>
            ...
            >
              {recommendedProducts.map((recProduct) => (
                <div
                  ...
                  style={{ flexShrink: 0, width: '360px', cursor: 'pointer' }}
                >
                  <div style={{ width: '100%', aspectRatio: '3/4', backgroundColor: '#f8f8f8', overflow: 'hidden', marginBottom: '20px' }}>
                    <img
                  ...
                  <p style={{
                    fontSize: '14px', letterSpacing: '1.5px', color: '#000',
      ...
      <style>{`
        .product-images-container::-webkit-scrollbar { display: none; }
        .product-images-container { -ms-overflow-style: none; scrollbar-width: none; }
        @media (min-width: 1280px) {
          .desktop-layout { grid-template-columns: 1fr 1fr !important; }
